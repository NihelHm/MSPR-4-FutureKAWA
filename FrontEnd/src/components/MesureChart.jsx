...
export default function MesureChart({ data, label, unit, color, seuilMin, seuilMax }) {
    ...
    const loadChart = async () => {
      ...
      chartRef.current = new Chart(ctx, {
        ...
        options: {
          ...
          scales: {
            x: { grid: { color: "#1E1E1E" }, ticks: { color: "#666", font: { size: 10 }, maxTicksLimit: 8 } },
            y: { grid: { color: "#1E1E1E" }, ticks: { color: "#666", font: { size: 11 }, callback: (v) => `${v}${unit}` } },
          },
        },
      });
    };
  ...
  if (!data || data.length === 0) {
    return <div className={styles.empty}><span>Aucune mesure disponible</span></div>;
  }

  const lastVal = data[0]?.valeur;
  const isAlert =
    lastVal !== undefined && seuilMin !== undefined && seuilMax !== undefined &&
    ...
    <div className={`${styles.wrapper} ${isAlert ? styles.alert : ""}`}>
      <div className={styles.header}>
        <div className={styles.info}>
          ...
          {seuilMin !== undefined && (
            <span className={styles.seuil}>Plage idéale : {seuilMin}–{seuilMax}{unit}</span>
          )}
        </div>
      ...
      <div className={styles.chartArea}><canvas ref={canvasRef} /></div>
    </div>
  );
}
